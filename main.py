...
class GCNII(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, layers, dropout, lamda, alpha, variant):
        super().__init__()
        self.convs = nn.ModuleList([GCNIIConv(hidden_dim, variant=variant) for _ in range(layers)])
        self.fcs = nn.ModuleList([nn.Linear(in_dim, hidden_dim), nn.Linear(hidden_dim, out_dim)])
...
def load_dataset(data_id, args):
    ...
    if data_id == 0:
        dataset = Planetoid(root=args.root + '/Cora', name='Cora', split=args.split, num_train_per_class=args.train_per_class, num_val=args.num_val, num_test=args.num_test, transform=transform)
    elif data_id == 1:
        dataset = Planetoid(root=args.root + '/Citeseer', name='CiteSeer', split=args.split, num_train_per_class=args.train_per_class, num_val=args.num_val, num_test=args.num_test, transform=transform)
    elif data_id == 2:
        dataset = Planetoid(root=args.root + '/Pubmed', name='PubMed', split=args.split, num_train_per_class=args.train_per_class, num_val=args.num_val, num_test=args.num_test, transform=transform)
    elif data_id == 3:
        dataset = WikipediaNetwork(root=args.root + '/Chameleon', name='chameleon', transform=transform)
    elif data_id == 4:
        dataset = WikipediaNetwork(root=args.root + '/Squirrel', name='squirrel', transform=transform)
    elif data_id == 5:
        dataset = Actor(root=args.root + '/Actor', transform=transform)
    elif data_id == 6:
        dataset = WebKB(root=args.root + '/Cornell', name='Cornell', transform=transform)
    elif data_id == 7:
        dataset = WebKB(root=args.root + '/Texas', name='Texas', transform=transform)
    elif data_id == 8:
        dataset = WebKB(root=args.root + '/Wisconsin', name='Wisconsin', transform=transform)
    else:
    ...
    if hasattr(data, 'train_mask') and data.train_mask is not None:
        ...
        data = random_balanced_split(data, class_count, args.train_per_class, args.num_val, args.num_test, args.seed)
...
def sparse_norm_adj(edge_index, num_nodes, device, dropedge=0.0, training=False):
...
def build_model(args, in_dim, out_dim):
    if args.model == 'gcnii':
        return GCNII(in_dim, args.hidden, out_dim, args.layers, args.dropout, args.lamda, args.alpha, args.variant)
    if args.model == 'appnp':
        return APPNPNet(in_dim, args.hidden, out_dim, args.mlp_layers, args.propagation_steps, args.alpha, args.dropout)
    if args.model == 'dagnn':
        return DAGNNNet(in_dim, args.hidden, out_dim, args.mlp_layers, args.propagation_steps, args.dropout)
    if args.model == 'gpr':
        return GPRNet(in_dim, args.hidden, out_dim, args.mlp_layers, args.propagation_steps, args.alpha, args.dropout, args.gpr_init)
...
def parse_args():
    parser = argparse.ArgumentParser(description='fast homophily baseline')
    parser.add_argument('data', type=int)
    parser.add_argument('--model', type=str, default='auto', choices=['auto', 'gcnii', 'appnp', 'dagnn', 'gpr'])
    ...
    parser.add_argument('--selection', type=str, default=None, choices=['val_loss', 'val_acc', 'hybrid'])
